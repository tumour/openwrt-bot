...
Нарушение dependency rule = архитектура сломана — поэтому правило проверяется
машинно: тест `internal/archtest` парсит импорты всех файлов domain- и app-слоёв
и падает при нарушении (гоняется в CI вместе с остальными тестами).
