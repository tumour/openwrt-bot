...
- **По итогам ревью — порт `Atomic` (unit of work)**: check-then-act из нескольких операций портов был гонкой (два админа одновременно удаляют друг друга → оба проходят guard «не последний» → бот без админа). Теперь мутирующий use case выполняет проверки и записи одной единицей: `store.Update(ctx, fn)` — JSON-адаптер работает над снапшотом под локом и пишет файлы только при успехе fn (ошибка = rollback, как в будущем SQL-адаптере); контрактный сьют проверяет rollback и сериализацию read-modify-write. Побочный выигрыш: одиночные операции портов стали транзакциями из одного действия — вторая механика хранения исчезла. Страховка от локаута задокументирована в Seed: данные без единого админа лечатся удалением env-админа из users.json + рестартом. Также по ревью: удалён мёртвый пакет-дубликат движка, исправлен двойной answerCallbackQuery в /users, README догнал archtest.
